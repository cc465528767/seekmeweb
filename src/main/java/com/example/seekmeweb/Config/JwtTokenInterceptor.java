...
/**
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object o) throws Exception {
        ...
        if(request.getRequestURI().equals("/jwt/token") || RequestMethod.OPTIONS.toString().equals(request.getMethod()))
        {
            return true;
        }
        ...
        try {
            ...
            if (authHeader == null || authHeader.trim() == "") {
                throw new SignatureException(Result.create(2,"查找失败",null));
            }
            ...
            //从数据库中获取token
            TokenInfo token = getDAO(TokenJPA.class,request).findOne(new Specification<TokenInfo>() {
                @Override
                public Predicate toPredicate(Root<TokenInfo> root, CriteriaQuery<?> criteriaQuery, CriteriaBuilder criteriaBuilder) {
                    criteriaQuery.where(criteriaBuilder.equal(root.get("phone1"), claims.getSubject()));
                    return null;
                }
            });
            //数据库中的token值
        ...
        catch (SignatureException | ExpiredJwtException e)
        {
            ...
            //输出error消息
            writer.write(Result.create(2,"tokennotvalue",null));
            writer.close();
            return false;
        }
        //出现异常时
